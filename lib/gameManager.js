...
const gameChats = new Map(); // gameId -> [{userId, displayName, message, timestamp}]

const MAX_CHAT_MESSAGES = 50; // チャットメッセージの最大保持数
...
export function deleteGame(gameId) {
    ...
    gameChats.delete(gameId); // チャットも削除
}
...
// チャット機能
export function addChatMessage(gameId, userId, displayName, message) {
    if (!gameChats.has(gameId)) {
        gameChats.set(gameId, []);
    }
    const messages = gameChats.get(gameId);
    messages.push({
        userId,
        displayName,
        message: message.slice(0, 200), // 文字数制限
        timestamp: Date.now()
    });
...
export function getChatMessages(gameId, since = 0) {
    const messages = gameChats.get(gameId) || [];
